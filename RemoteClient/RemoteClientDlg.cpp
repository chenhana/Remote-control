...
void CRemoteClientDlg::OnBnClickedBtnTest()
{
	CClientController::getInstance()->
		SendCommandPacket(GetSafeHwnd() ,1981);
...
void CRemoteClientDlg::InitUIData()
{
	...
	// TODO: 在此添加额外的初始化代码
	UpdateData();
	//m_server_address = 0xC0A8018D;   //192.168.1.141
	...
	m_dlgStatus.Create(IDD_DLG_STATUS, this); //非模态
...
void CRemoteClientDlg::Str2Tree(const std::string & drivers, CTreeCtrl& tree)
{
	std::string dr;
	tree.DeleteAllItems();
	for (size_t i = 0; i < drivers.size(); i++) //驱动信息
	{
		if (drivers[i] == ',') {
			dr += ":";
...
void CRemoteClientDlg::LoadFileInfo()
{
	...
	HTREEITEM hTreeSelected = m_Tree.HitTest(ptMouse, 0); //双击
	...
	std::list<CPacket> lstPackets;
	int cCmd = CClientController::getInstance()->SendCommandPacket(GetSafeHwnd() ,2, false, 
...
LPARAM CRemoteClientDlg::OnSendPacketAck(WPARAM wParam, LPARAM lParam)
{
